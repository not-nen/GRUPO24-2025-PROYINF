...
// import { MAX_PRIMER_PAGO, MIN_PRIMER_PAGO, MIN_MONTO, MAX_MONTO, MAX_PLAZO } from "schemas/schema";
...
Credito.Form = function CreditoForm() {
    ...
    const handlePlazo = (e) => {
        handleChange(e);
        const value = e.target.value;
        if (value > MAX_PLAZO || value.length > MAX_PLAZO.toString().length)
            setFieldValue("plazo_otro", values.plazo_otro);
        else setFieldValue("plazo_otro", value);
    };
...
Credito.Buttons = function CreditoButtons() {
    return (
        <BtnsContainer>
            <button type="submit" className="btn btn-primary">
