...
import BtnsContainer from "components/containers/BtnsContainer";
import { useAuth } from "context/authContext";
import { useSimulacion, SimulacionProvider } from "context/simulacionContext";

const Simulacion = () => null;

Simulacion.Form = function SimulacionForm() {
    const { dataFetch, error, loading } = useSimulacion();
    ...
    return (
        <div className="d-flex flex-column px-4 h-100 overflow-auto">
            <h2 className="mb-4 text-center krona-one-regular">Resultado de tu Simulación</h2>
            ...
            {!loading && dataFetch && (
                <div className="animate__animated animate__fadeIn">
                    ...
                    <div className="card bg-light border-0 rounded-4 p-4 mt-2">
                        <h5 className="mb-3">Detalles del Crédito</h5>
                        <div className="row g-3">
                            <div className="col-6 col-md-4">
...
Simulacion.Buttons = function SimulacionButtons() {
    const navigate = useNavigate();
    const { isAuthenticated } = useAuth();
    const { handleGuardar } = useSimulacion();

    const guardar = async () => {
        const res = await handleGuardar();
    ...
    return (
        <BtnsContainer>
            {isAuthenticated && (
                <button className="btn btn-success btn-top" onClick={guardar}>
                    Guardar Simulación
                </button>
            )}
...
Simulacion.Provider = SimulacionProvider;
